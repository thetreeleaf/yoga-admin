      ...
      <el-dialog
        ...
        <el-form
          ...
          <el-form-item label="URL权限标识" prop="urlPerm">
            <el-input placeholder="/sysUser/*" v-model="urlPerm.requestPath" class="input-with-select">
              <el-select v-model="urlPerm.serviceName" style="width: 130px;" slot="prepend" placeholder="所属服务"
                         clearable>
                <el-option v-for="item in microServiceList" :value="item.name" :label="item.name" :key="item.id"/>
              </el-select>
              <el-select v-model="urlPerm.requestMethod" style="width: 120px;margin-left: 20px" slot="prepend"
                         placeholder="请求方式" clearable>
                <el-option v-for="item in requestMethodList" :value="item.value" :label="item.name" :key="item.id"/>
              </el-select>
            </el-input>
...
import {microServiceList} from "@/api/system/menu";
  ...
  data() {
    return {
      ...
      requestMethodList: [
        ...
        { id: 3, value: "PUT", name: "PUT"},
        { id: 4, value: "DELETE", name: "DELETE"},
        { id: 5, value: "PATCH", name: "PATCH"},
      ]
    }
  },
  methods: {
    ...
    loadMicroServices() {
      microServiceList().then(response => {
